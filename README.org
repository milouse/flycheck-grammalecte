...
If you like my work, you can help me a lot by giving me some tip through
...
more how to install a package.

Otherwise, just clone this repository somewhere and load necessary files from
your GNU Emacs config file:
...
You can also add it to your Gnu Emacs config file, but be warned that it
might introduce some delay when starting Emacs, due to the http request
made to check the grammalecte version and eventually upgrade of the
package:
...
~C-c ! g~ by default. You can also right-click on it and a menu will
popup to let you choose the better correction.

If you are looking for a better word at some place, you can use the
function ~grammalecte-find-synonyms-at-point~ (which will take the word
at point to find synonyms and antonyms) or the simpler
~grammalecte-find-synonyms~ (which will ask you for a word before doing
the search). These functions are always available, even in buffer, which
major mode is not in the ~flycheck-grammalecte-enabled-modes~ list.
...
You can also set these variables in your GNU Emacs config file with
~setopt~. This can be done before or after the
~flycheck-grammalecte-setup~ call.

#+begin_src emacs-lisp
  (with-eval-after-load 'flycheck
    (flycheck-grammalecte-setup)
    (setopt flycheck-grammalecte-report-esp nil)
...
#+begin_src emacs-lisp
  (use-package flycheck-grammalecte :defer t
    :hook (fountain-mode . flycheck-grammalecte-setup)
    :custom
